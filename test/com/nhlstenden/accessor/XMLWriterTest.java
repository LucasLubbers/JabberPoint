...
class XMLWriterTest {
  ...
  void testSaveFile() throws IOException {
    ...
    assertTrue(
        lines.stream().anyMatch(line -> line.contains("<showtitle>Writer Test</showtitle>")));
    assertTrue(
        lines.stream()
            .anyMatch(line -> line.contains("<item kind=\"text\" level=\"1\">Some text</item>")));
    assertTrue(
        lines.stream()
            .anyMatch(line -> line.contains("<item kind=\"image\" level=\"2\">image.png</item>")));
  }
