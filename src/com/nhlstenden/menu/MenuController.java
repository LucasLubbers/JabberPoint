...
public class MenuController {
  ...
  private Menu createFileMenu() {
    ...
    fileMenu.add(
        MenuItemFactory.createMenuItem(
            OPEN, new OpenFileCommand(presentation, parent, slideViewerFrame), "O"));
    fileMenu.add(
        MenuItemFactory.createMenuItem(
            NEW, new NewPresentationCommand(presentation, parent, slideViewerFrame), "N"));
    fileMenu.add(
        MenuItemFactory.createMenuItem(
            NEW_SLIDE, new SetNewSlideCommand(presentation, parent), "S"));
    fileMenu.add(
        MenuItemFactory.createMenuItem(SAVE, new SaveFileCommand(presentation, parent), "S"));
  ...
  private Menu createViewMenu() {
    ...
    viewMenu.add(
        MenuItemFactory.createMenuItem(GOTO, new GoToSlideCommand(presentation, parent), "G"));
    return viewMenu;
  }
  ...
  private Menu createEditMenu() {
    ...
    editMenu.add(
        MenuItemFactory.createMenuItem(
            ADD_TEXT, new AddTextItemCommand(presentation, parent), "T"));
    editMenu.add(
        MenuItemFactory.createMenuItem(
            ADD_IMAGE, new AddBitmapItemCommand(presentation, parent), "I"));
    editMenu.add(
        MenuItemFactory.createMenuItem(
            SAVE_STATE, new SaveStateCommand(presentation, parent, careTaker), "F5"));
    editMenu.add(
        MenuItemFactory.createMenuItem(
            RESTORE_STATE, new RestoreStateCommand(presentation, parent, careTaker), "F9"));
    editMenu.add(
        MenuItemFactory.createMenuItem(
            DELETE_SLIDE, new DeleteSlideCommand(presentation, parent), "D"));
    return editMenu;
  }
